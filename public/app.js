...
function handleDeleteProject() {
    ...
    // Cannot delete project while timer is running
    if (currentTimer) {
        alert('Cannot delete project while timer is running. Please stop the timer first.');
        return;
    }
    
    // Show confirmation dialog
    if (!confirm(`Are you sure you want to delete "${currentProject}"? This will mark all associated time entries as "Deleted Project".`)) {
        return;
    }
    ...
    const option = projectSelect.querySelector(`option[value="${currentProject}"]`);
    if (option) {
        option.remove();
    }
    
    // Update time entries to show "Deleted Project"
    timeEntries.forEach(entry => {
        if (entry.project === currentProject) {
            entry.project = 'Deleted Project';
        }
    });
    
    // Store project name for toast before clearing
    const deletedProjectName = currentProject;
    ...
    showToast(`Project "${deletedProjectName}" has been deleted`);
}
